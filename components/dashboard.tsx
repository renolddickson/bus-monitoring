...
export function Dashboard({ buses, routes, busLocations, onSelectBus, loading }: DashboardProps) {
  const [activeBuses, setActiveBuses] = useState<Array<Bus & { 
  ...
  useEffect(() => {
    // Process the data to combine bus info with locations and routes
    if (buses.length && routes.length && busLocations.length) {
      const processedBuses = buses.map(bus => {
        ...
        // Calculate ETA (in minutes) and progress (0-100%)
        let eta = 0
        let progress = 0
        
        if (location && route && route.stops.length > 0) {
          // Find the next stop
          const nextStopIndex = route.stops.findIndex(stop => 
            stop.order > location.lastPassedStopOrder
          )
          
          if (nextStopIndex !== -1) {
            const nextStop = route.stops[nextStopIndex]
            // Simplified ETA calculation (in real app would use distance calculation)
        ...
        <Card className="bg-white shadow-sm">
          ...
          <CardContent>
            <div className="flex items-baseline justify-between">
              <div className="text-3xl font-bold">{activeBuses.filter(b => b.location && b.location.active).length}</div>
              <Badge variant="outline" className="bg-green-50 text-green-700 border-green-200">
                {Math.round((activeBuses.filter(b => b.location && b.location.active).length / activeBuses.length) * 100)}% Fleet Active
              </Badge>
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent>
            <div className="flex items-baseline justify-between">
              ...
              <Badge variant="outline" className="bg-blue-50 text-blue-700 border-blue-200">
                {routes.reduce((acc, route) => acc + route.stops.length, 0)} Stops
              </Badge>
            </div>
          </CardContent>
        </Card>
        ...
        <CardContent className="p-6">
          <div className="space-y-4">
            {activeBuses.slice(0, 5).map((bus, index) => (
              <div key={`activity-${bus.id}-${index}`} className="flex items-center pb-4 border-b border-gray-100 last:border-0">
